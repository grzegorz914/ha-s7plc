...
def _item_has_required_fields(option_key: str, item: Mapping[str, Any]) -> bool:
    ...
    if option_key == CONF_COVERS:
        if item.get(CONF_POSITION_STATE_ADDRESS):
            return True
        # toggle_mode covers are single-pulse: close_command_address is
        # never set for them (see config_validation.py), so requiring it
        # unconditionally would wrongly treat every real toggle_mode cover
        # as unexpected - matches cover.py's actual creation check.
        toggle_mode = bool(item.get(CONF_TOGGLE_MODE, DEFAULT_TOGGLE_MODE))
        return bool(
            item.get(CONF_OPEN_COMMAND_ADDRESS)
            and (item.get(CONF_CLOSE_COMMAND_ADDRESS) or toggle_mode)
        )

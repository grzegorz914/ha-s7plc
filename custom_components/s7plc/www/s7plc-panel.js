...
.map(([k,v])=>{
  ...
  addressField(key,value,label,description,required,type){const allowed=ADDRESS_TYPES_FOR_FIELD(type,key),parsed=PARSE_S7_ADDRESS(value),guided=!parsed.error&&(!parsed.dataType||allowed.includes(parsed.dataType)),option=([dataType,,text])=>allowed.includes(dataType)?`<option value="${dataType}" ${parsed.dataType===dataType?'selected':''}>${text}</option>`:'';return `<fieldset class="address-builder" data-field="${key}" data-address-builder data-required="${required?'true':'false'}" tabindex="-1"><legend><span class="label-text">${this.escape(label)}</span>${required?`<em>${this.t('common.required')}</em>`:''}</legend><input type="hidden" name="${key}" value="${this.escape(value)}"><div class="address-modes"><button type="button" data-address-mode="guided" class="${guided?'active':''}">${this.t('address_builder.guided')}</button><button type="button" data-address-mode="manual" class="${guided?'':'active'}">${this.t('address_builder.manual')}</button></div><div class="address-guided" ${guided?'':'hidden'}><div class="address-controls"><label>${this.t('address_builder.area')}<select data-address-part="area"><option value=""></option>${["DB","I","Q","M"].map(area=>`<option value="${area}" ${parsed.area===area?'selected':''}>${area}</option>`).join('')}</select></label><label data-db-number>${this.t('address_builder.db_number')}<input type="number" min="0" step="1" data-address-part="dbNumber" value="${parsed.dbNumber??''}"></label><label>${this.t('address_builder.data_type')}<select data-address-part="dataType"><option value=""></option>${S7_ADDRESS_TYPES.map(option).join('')}</select></label><label>${this.t('address_builder.offset')}<input type="number" min="0" step="1" data-address-part="offset" value="${parsed.offset??''}"></label><label data-bit>${this.t('address_builder.bit')}<input type="number" min="0" max="7" step="1" data-address-part="bit" value="${parsed.bit??''}"></label><label data-length>${this.t('address_builder.length')}<input type="number" min="0" step="1" data-address-part="length" value="${parsed.length??''}"></label></div><div class="address-preview"><span>${this.t('address_builder.preview')}</span><code>${this.escape(value)}</code></div></div><label class="address-manual" ${guided?'hidden':''}><input type="text" class="mono" data-address-manual value="${this.escape(value)}" placeholder="${this.t(`common.${ADDRESS_PLACEHOLDER_KEY(allowed)}`)}"></label><small class="address-error" role="alert" ${parsed.error?'':'hidden'}>${this.t(`address_builder.${parsed.error||'invalid'}`)}</small>${description?`<small class="address-help">${this.escape(description)}</small>`:''}</fieldset>`;}
  ...
  field.querySelectorAll('[data-address-mode]').forEach(button=>button.onclick=()=>{if(button.dataset.addressMode==='manual'){manual.value=hidden.value;setMode('manual');return;}const parsed=PARSE_S7_ADDRESS(manual.value);if(parsed.error||parsed.dataType&&!allowed.includes(parsed.dataType)){setMode('manual');showError(parsed.error||'unsupported');return;}hidden.value=manual.value;writeParts(parsed);setMode('guided');showError('');});
...
.address-builder{container-type:inline-size;grid-column:1/-1;min-width:0;margin:0;padding:14px;border:1px solid var(--divider-color);border-radius:14px}.address-builder legend{display:flex;align-items:center;gap:10px;padding:0 5px;font-weight:600}.address-builder legend em{font-size:9px;font-style:normal;text-transform:uppercase;letter-spacing:.06em;color:var(--primary-color)}.address-modes{display:inline-flex;gap:3px;margin:2px 0 10px;padding:3px;border-radius:10px;background:var(--secondary-background-color)}.address-modes button{padding:7px 14px;border-radius:8px;font-weight:600;opacity:.7}.address-modes button.active{color:var(--primary-color);background:var(--card-background-color);box-shadow:0 1px 4px #00000024;opacity:1}.address-controls{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,150px),1fr));gap:10px 12px}.address-controls label{min-width:0}.address-controls input,.address-controls select,.address-manual input{box-sizing:border-box;width:100%;min-width:0}.address-preview{display:flex;align-items:baseline;justify-content:space-between;gap:14px;margin-top:10px;padding:9px 12px;border-radius:10px;background:var(--secondary-background-color)}.address-preview code{min-width:0;overflow-wrap:anywhere;text-align:right}.address-manual{margin-top:2px}.address-error,.address-help{display:block;margin-top:8px}.address-error{color:var(--error-color)!important}.address-builder:focus{outline:none}.address-builder[data-address-error]:not([data-address-error=""]){border-color:var(--error-color)}.address-builder[data-address-error]:not([data-address-error=""]):focus{outline:2px solid var(--error-color);outline-offset:2px}.address-builder [hidden]{display:none!important}
@container(min-width:850px){.address-controls{grid-template-columns:repeat(5,minmax(0,1fr))}}
...
.options-map{grid-column:1/-1;border:1px solid var(--divider-color);border-radius:12px;padding:12px 14px 14px;margin:0;display:flex;flex-direction:column;gap:8px}
...
.om-head{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr) 40px;gap:8px;font-size:12px;color:var(--secondary-text-color);padding:0 2px}
.om-rows{display:flex;flex-direction:column;gap:8px}
.om-row{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr) 40px;gap:8px;align-items:center}
...
.om-del:hover{color:var(--error-color);border-color:var(--error-color)}
...
@media(max-width:650px){.dialog-body{max-height:66vh;padding:0 14px 16px}.editor-intro p{display:none}.form-section{padding:14px 12px}.field-grid{grid-template-columns:1fr}.address-controls{grid-template-columns:1fr}.control-options{grid-template-columns:1fr}.control-card{min-height:0;flex-direction:row!important}.control-card.compact-control-card{min-height:110px;flex-direction:column!important}.mode-tabs button{font-size:12px}.mode-tabs button small{display:none}.availability-stats{grid-template-columns:repeat(2,minmax(0,1fr))}.connection-detail{gap:10px;padding:10px 12px}.connection-detail dt{white-space:normal}}`;}
  }

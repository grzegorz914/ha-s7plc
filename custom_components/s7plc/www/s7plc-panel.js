...
const COVER_UI_FROM_ENTITY = entity => {
  ...
  const legacyPositionFeedback=entity.use_state_topics===false?"timed":entity.opening_state_address&&entity.closing_state_address?"both":entity.opening_state_address?"opening":entity.closing_state_address?"closing":"timed";
  const positionFeedback=control==="traditional"?(entity.cover_position_feedback||legacyPositionFeedback):"timed";
  const movementFeedback=entity.cover_status_address&&!(control==="traditional"&&positionFeedback==="status")?"status":control==="traditional"&&positionFeedback!=="status"&&(entity.cover_opening_address||entity.cover_closing_address||entity.cover_stopped_address)?"bits":"none";
...
const COVER_POSITION_FIELDS=["position_state_address","position_command_address","invert_position","position_tilt_bidirectional","tilt_state_address","tilt_command_address","invert_tilt","stop_command_address","stop_pulse_duration"];
...
const CLEAN_COVER_ENTITY=(source,ui)=>{const entity={...source};COVER_VIRTUAL_FIELDS.forEach(key=>delete entity[key]);if(ui.preserve_legacy&&ui.cover_control_mode==="traditional"){COVER_POSITION_FIELDS.forEach(key=>delete entity[key]);delete entity.cover_position_feedback;return entity;}if(ui.cover_control_mode==="traditional"){COVER_POSITION_FIELDS.forEach(key=>delete entity[key]);const mode=ui.cover_position_feedback;entity.cover_position_feedback=mode;if(mode==="status"){delete entity.use_state_topics;delete entity.opening_state_address;delete entity.closing_state_address;["cover_opening_address","cover_closing_address","cover_stopped_address"].forEach(key=>delete entity[key]);}else{if(ui.cover_movement_feedback!=="status")["cover_status_address",...COVER_STATUS_VALUE_FIELDS].forEach(key=>delete entity[key]);if(mode==="timed"){entity.use_state_topics=false;delete entity.opening_state_address;delete entity.closing_state_address;}else{entity.use_state_topics=true;if(mode==="opening")delete entity.closing_state_address;if(mode==="closing")delete entity.opening_state_address;}}}else{COVER_TRADITIONAL_FIELDS.forEach(key=>delete entity[key]);delete entity.cover_position_feedback;}
  const traditionalStatus=ui.cover_control_mode==="traditional"&&ui.cover_position_feedback==="status";if(ui.cover_movement_feedback==="none"&&!traditionalStatus){["cover_opening_address","cover_closing_address","cover_stopped_address","cover_status_address",...COVER_STATUS_VALUE_FIELDS].forEach(key=>delete entity[key]);}else if(ui.cover_movement_feedback==="bits"){["cover_status_address",...COVER_STATUS_VALUE_FIELDS].forEach(key=>delete entity[key]);}else {["cover_opening_address","cover_closing_address","cover_stopped_address"].forEach(key=>delete entity[key]);}
...
class S7PlcConfigurationPanel extends HTMLElement {
  ...
  openEditor(index=null,type=this.type||TYPES[0]){const entry=this.entries.find(e=>e.entry_id===this.entryId),raw=index===null?{}:entry.entities[type][index],initial=this.inferred(raw,type),dialog=document.createElement('ha-dialog');dialog.open=true;dialog.headerTitle=index===null?this.t('editor.new_entity'):this.t('editor.edit_entity');dialog.style.setProperty('--mdc-dialog-max-width','min(940px,95vw)');dialog.style.setProperty('--mdc-dialog-min-width','min(940px,95vw)');dialog.style.setProperty('--dialog-content-padding','0');dialog.innerHTML=`<style>${this.dialogStyles}</style><div class="dialog-body"><div class="editor-intro"><span class="editor-type-icon"><ha-icon icon="mdi:${this.icon(type)}"></ha-icon></span><div><span class="eyebrow">${this.t(`entity_types.${type}.label`)}</span><h3>${index===null?this.t('editor.configure_new'):this.t('editor.update_configuration')}</h3><p>${this.t('editor.save_help')}</p></div></div><div class="mode-tabs" role="tablist" aria-label="${this.t('editor.editor_mode')}"><button class="active" data-mode="visual" role="tab"><ha-icon icon="mdi:form-select"></ha-icon><span>${this.t('editor.visual_editor')}<small>${this.t('editor.guided')}</small></span></button><button data-mode="yaml" role="tab"><ha-icon icon="mdi:code-braces"></ha-icon><span>YAML<small>${this.t('editor.advanced')}</small></span></button></div><form class="visual-form">${this.editorSections(type,initial)}</form><div class="yaml-editor" style="display:none"><ha-alert alert-type="warning">${this.t('editor.yaml_warning')}</ha-alert><textarea spellcheck="false" aria-label="${this.t('editor.yaml_label')}">${this.escape(this.toYaml(raw))}</textarea></div><ha-alert class="editor-error" alert-type="error" style="display:none"></ha-alert></div><ha-dialog-footer slot="footer"><ha-button slot="secondaryAction" appearance="plain">${this.t('actions.cancel')}</ha-button><ha-button slot="primaryAction" appearance="accent">${this.t('actions.save')}</ha-button></ha-dialog-footer>`;document.body.appendChild(dialog);
    ...
    const syncMode=()=>{if(type==='covers'){const control=form.elements.cover_control_mode.value,positionFeedback=form.elements.cover_position_feedback.value,movement=form.elements.cover_movement_feedback.value,stop=form.elements.cover_stop_enabled.value==='enabled',tilt=form.elements.cover_tilt_enabled.value==='enabled';const visible=new Set(control==='position'?['position_state_address','position_command_address','invert_position','position_tilt_bidirectional']:['open_command_address','close_command_address']);if(control==='traditional')visible.add('operate_time');if(control==='traditional'&&['opening','both'].includes(positionFeedback))visible.add('opening_state_address');if(control==='traditional'&&['closing','both'].includes(positionFeedback))visible.add('closing_state_address');if(control==='traditional'&&positionFeedback==='status')['cover_status_address',...COVER_STATUS_VALUE_FIELDS].forEach(k=>visible.add(k));if(movement==='bits'&&control==='traditional')['cover_opening_address','cover_closing_address','cover_stopped_address'].forEach(k=>visible.add(k));if(movement==='status')['cover_status_address',...COVER_STATUS_VALUE_FIELDS].forEach(k=>visible.add(k));if(control==='position'&&stop)['stop_command_address','stop_pulse_duration'].forEach(k=>visible.add(k));if(control==='position'&&tilt)['tilt_state_address','tilt_command_address','invert_tilt'].forEach(k=>visible.add(k));const governed=[...COVER_TRADITIONAL_FIELDS,...COVER_POSITION_FIELDS,'cover_status_address',...COVER_STATUS_VALUE_FIELDS,'stop_command_address','stop_pulse_duration'];governed.forEach(k=>form.querySelector(`[data-field="${k}"]`)?.classList.toggle('hidden-field',!visible.has(k)));form.querySelector('[data-section="cover-position-feedback"]').classList.toggle('hidden-field',control==='position');form.querySelector('[data-section="cover-stop"]').classList.toggle('hidden-field',control!=='position');form.querySelector('[data-section="cover-tilt"]').classList.toggle('hidden-field',control!=='position');form.querySelector('[data-section="cover-position-tilt-feedback"]').classList.toggle('hidden-field',control!=='position');const statusMovement=form.querySelector('input[name="cover_movement_feedback"][value="status"]');statusMovement.closest('.control-card').classList.toggle('hidden-field',control==='traditional'&&positionFeedback==='status');if(control==='traditional'&&positionFeedback==='status'&&statusMovement.checked)form.querySelector('input[name="cover_movement_feedback"][value="none"]').checked=true;const bits=form.querySelector('input[name="cover_movement_feedback"][value="bits"]');bits.closest('.control-card').classList.toggle('hidden-field',control==='position');if(control==='position'&&bits.checked)form.querySelector('input[name="cover_movement_feedback"][value="none"]').checked=true;return;}if(type==='climates'){const value=name=>form.querySelector(`input[name="${name}"]:checked`)?.value,{fields,sections}=CLIMATE_EDITOR_VISIBILITY({control_mode:value('control_mode'),direct_function:value('climate_direct_function'),direct_feedback:value('climate_direct_feedback'),mode_control:value('climate_mode_control'),action_feedback:value('climate_action_feedback'),availability_mode:value('availability_mode')});form.querySelectorAll('[data-field]').forEach(field=>field.classList.toggle('hidden-field',!fields.has(field.dataset.field)));form.querySelectorAll('[data-section]').forEach(section=>section.classList.toggle('hidden-field',!sections.has(section.dataset.section)));return;}const sel=form.elements.control_mode;if(!sel)return;const hidden=MODE_HIDDEN[type]?.[sel.value]||[];form.querySelectorAll('[data-field]').forEach(l=>l.classList.toggle('hidden-field',hidden.includes(l.dataset.field)));};

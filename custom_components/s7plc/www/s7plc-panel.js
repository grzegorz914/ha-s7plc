...
const FIELDS = {
  sensors:[["address","Indirizzo PLC","text",true],["device_class","Classe dispositivo"],["unit_of_measurement","Unità di misura"],["state_class","Classe di stato"],["real_precision","Decimali","number"],...COMMON],
...
class S7PlcConfigurationPanel extends HTMLElement {
  ...
  async refreshConnectionStatus(){
    ...
    try{this.entries=await this._hass.callWS({type:"s7plc/config/list"});const entry=this.entries.find(e=>e.entry_id===this.entryId),badge=this.querySelector('.connection-badge');if(entry&&badge){badge.classList.toggle('connected',entry.connected);badge.textContent=this.t(entry.connected?'connected':'disconnected');}}
  ...
  availabilityMarkup(result){const label=state=>this.t(`availability.${state}`),timeline=result.intervals.map(interval=>{const width=(interval.end-interval.start)/CONNECTION_WINDOW_MS*100,title=`${label(interval.state)} · ${this.formatDuration(interval.end-interval.start)}`;return `<span class="timeline-segment ${interval.state}" style="width:${width}%" title="${this.escape(title)}" aria-label="${this.escape(title)}"></span>`;}).join(''),last=result.lastDisconnection,unknown=result.durations.unknown;return `<section class="availability"><div class="availability-title"><b>${this.t('availability.title')}</b><small>${this.t('availability.last_24_hours')}</small></div><div class="connection-timeline" role="img" aria-label="${this.t('availability.timeline_label')}">${timeline}</div><div class="timeline-labels"><span>${this.t('availability.hours_ago')}</span><span>${this.t('availability.now')}</span></div><dl class="availability-stats"><div><dt>${this.t('availability.percentage')}</dt><dd>${result.availability===null?'—':`${result.availability.toFixed(1)}%`}</dd></div><div><dt>${this.t('availability.current_uptime')}</dt><dd>${this.formatDuration(result.currentUptime)}</dd></div><div><dt>${this.t('availability.disconnections')}</dt><dd>${result.disconnects}</dd></div><div><dt>${this.t('availability.unknown_time')}</dt><dd>${this.formatDuration(unknown)}</dd></div></dl>${last?`<p class="last-disconnection"><b>${this.t('availability.last_disconnection')}</b><span>${new Intl.DateTimeFormat(this.language,{dateStyle:'short',timeStyle:'short'}).format(last.start)} · ${this.formatDuration(last.end-last.start)}</span></p>`:''}<small class="availability-note">${this.t('availability.percentage_note')}</small></section>`;}
  async loadConnectionAvailability(dialog,entry){const container=dialog.querySelector('.availability-container'),entityId=entry.connection_entity_id;if(!entityId){container.innerHTML=`<p class="history-unavailable">${this.t('availability.history_unavailable')}</p>`;return;}try{const now=Date.now(),start=new Date(now-CONNECTION_WINDOW_MS).toISOString(),end=new Date(now).toISOString(),path=`history/period/${encodeURIComponent(start)}?filter_entity_id=${encodeURIComponent(entityId)}&end_time=${encodeURIComponent(end)}&minimal_response&no_attributes`;const response=await this._hass.callApi('GET',path),history=Array.isArray(response?.[0])?response[0]:[];if(!history.length)throw Error('empty history');container.innerHTML=this.availabilityMarkup(BUILD_CONNECTION_AVAILABILITY(history,now));}catch(err){console.debug('Unable to load S7 PLC connection history',err);container.innerHTML=`<p class="history-unavailable">${this.t('availability.history_unavailable')}</p>`;}}
  openConnectionDetails(entry){
    const dialog=document.createElement('ha-dialog'),{host,port}=entry.data;
    const rows=Object.entries(entry.data).map(([key,value])=>`<div class="connection-detail"><dt>${this.escape(this.connectionLabel(key))}</dt><dd>${this.escape(this.connectionValue(value))}</dd></div>`).join('');
    ...
    dialog.innerHTML=`<style>${this.dialogStyles}</style><div class="dialog-body connection-details"><div class="connection-head"><div class="connection-head-text"><b>${this.escape(entry.title)}</b><code>${this.escape(host??'—')}${port!==undefined&&port!==null&&port!==''?`:${this.escape(port)}`:''}</code></div><span class="connection-status ${entry.connected?'connected':''}">${this.t(entry.connected?'connected':'disconnected')}</span></div><p>${this.t('connection_details_description')}</p><div class="availability-container"><div class="history-loading" role="status"><span></span>${this.t('availability.loading')}</div></div><dl>${rows}</dl></div><ha-dialog-footer slot="footer"><ha-button slot="primaryAction" appearance="accent">${this.t('close')}</ha-button></ha-dialog-footer>`;
  ...
  formEntity(form,original,type){if(!form.reportValidity())throw Error(this.t('required_error'));const entity={...original};for(const field of FIELDS[type]){const [key,,,required]=field;if(key==='control_behavior')continue;const input=form.elements[key];let value=input.type==='checkbox'?input.checked:input.value.trim();if(key==='cover_mode'||key==='control_mode')continue;if(input.type==='number'&&value!=='')value=Number(value);const presetModeValue=key.startsWith('preset_mode_')&&key.endsWith('_value'),statusCoreValue=key in CLIMATE_STATUS_CORE_DEFAULTS;if(value===''&&!required){if(presetModeValue)entity[key]=null;else if(statusCoreValue)entity[key]='';else delete entity[key];}else entity[key]=value;}if(type==='switches'||type==='lights')Object.assign(entity,APPLY_CONTROL_MODE({},form.elements.control_behavior.value));const mode=form.elements.cover_mode?.value;if(type==='covers'){MODE_HIDDEN.covers[mode].forEach(k=>delete entity[k]);if(mode==='position'&&!entity.tilt_state_address){COVER_TILT_INVERT_FIELDS.forEach(k=>delete entity[k]);}if(mode==='traditional'&&entity.toggle_mode){delete entity.close_command_address;}const needed=mode==='position'?'position_state_address':'open_command_address';if(!entity[needed]||(mode==='traditional'&&!entity.toggle_mode&&!entity.close_command_address))throw Error(this.t('cover_required_error'));}if(type==='climates'){entity.control_mode=form.elements.control_mode.value;MODE_HIDDEN.climates[entity.control_mode].forEach(k=>delete entity[k]);if(entity.control_mode==='setpoint'&&!entity.target_temperature_address)throw Error(this.t('climate_required_error'));}return entity;}
...
.details>b{font-size:14px;letter-spacing:-.01em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
.control-selector{grid-column:1/-1;border:0;padding:0;margin:0;min-width:0}.control-selector legend{font-size:13px;font-weight:600;margin-bottom:10px;padding:0}.control-options{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}
...
.connection-details>p{color:var(--secondary-text-color);font-size:12.5px;margin:12px 0 14px}
...
.connection-details dl{margin:0;border:1px solid var(--divider-color);border-radius:14px;overflow:hidden}
...
.connection-detail dt{color:var(--secondary-text-color);font-size:12.5px;white-space:nowrap}
.connection-detail dd{margin:0;font-size:13px;font-weight:600;text-align:right;overflow-wrap:anywhere;font-variant-numeric:tabular-nums;font-family:ui-monospace,'SF Mono',Consolas,monospace}

...
const COVER_UI_FROM_ENTITY = entity => {
  ...
  const positionFeedback=control==="traditional"&&(entity.opening_state_address||entity.closing_state_address||entity.use_state_topics)?"endstops":"timed";
  ...
  return {cover_control_mode:control,cover_position_feedback:positionFeedback,cover_movement_feedback:movementFeedback,cover_stop_enabled:Boolean(entity.stop_command_address),cover_tilt_enabled:Boolean(entity.tilt_state_address||entity.tilt_command_address)};
};
...
const COVER_POSITION_FIELDS=["position_state_address","position_command_address","invert_position","command_bidirectional","tilt_state_address","tilt_command_address","invert_tilt"];
const CLEAN_COVER_ENTITY=(source,ui)=>{const entity={...source};COVER_VIRTUAL_FIELDS.forEach(key=>delete entity[key]);if(ui.cover_control_mode==="traditional"){COVER_POSITION_FIELDS.forEach(key=>delete entity[key]);entity.use_state_topics=ui.cover_position_feedback==="endstops";if(ui.cover_position_feedback!=="endstops"){delete entity.opening_state_address;delete entity.closing_state_address;} }else{COVER_TRADITIONAL_FIELDS.forEach(key=>delete entity[key]);}
  ...
  if(!ui.cover_stop_enabled){delete entity.stop_command_address;delete entity.stop_pulse_duration;}if(!ui.cover_tilt_enabled){delete entity.tilt_state_address;delete entity.tilt_command_address;delete entity.invert_tilt;}return entity;};
...
const FIELDS = {
  ...
  climates:[["control_mode","select",true,["direct","setpoint"]],["current_temperature_address","text",true],["target_temperature_address"],["heating_output_address"],["cooling_output_address"],["heating_action_address"],["cooling_action_address"],["preset_mode_address"],["preset_mode_bidirectional","checkbox"],["on_off_address"],["preset_mode_off_value","number"],["preset_mode_heat_value","number"],["preset_mode_cool_value","number"],["preset_mode_heat_cool_value","number"],["preset_mode_auto_value","number"],["preset_mode_dry_value","number"],["preset_mode_fan_only_value","number"],["hvac_status_address"],["hvac_status_off_values"],["hvac_status_heating_values"],["hvac_status_cooling_values"],["hvac_status_idle_values"],["hvac_status_drying_values"],["hvac_status_fan_values"],["hvac_status_preheating_values"],["hvac_status_defrosting_values"],["min_temp","number"],["max_temp","number"],["temp_step","number"],...COMMON],
...
class S7PlcConfigurationPanel extends HTMLElement {
  ...
  openEditor(index=null){const entry=this.entries.find(e=>e.entry_id===this.entryId),type=this.type||TYPES[0],raw=index===null?{}:entry.entities[type][index],initial=this.inferred(raw,type),dialog=document.createElement('ha-dialog');dialog.open=true;dialog.headerTitle=index===null?this.t('editor.new_entity'):this.t('editor.edit_entity');dialog.style.setProperty('--mdc-dialog-max-width','min(940px,95vw)');dialog.style.setProperty('--mdc-dialog-min-width','min(940px,95vw)');dialog.style.setProperty('--dialog-content-padding','0');dialog.innerHTML=`<style>${this.dialogStyles}</style><div class="dialog-body"><div class="editor-intro"><span class="editor-type-icon"><ha-icon icon="mdi:${this.icon(type)}"></ha-icon></span><div><span class="eyebrow">${this.t(`entity_types.${type}.label`)}</span><h3>${index===null?this.t('editor.configure_new'):this.t('editor.update_configuration')}</h3><p>${this.t('editor.save_help')}</p></div></div><div class="mode-tabs" role="tablist" aria-label="${this.t('editor.editor_mode')}"><button class="active" data-mode="visual" role="tab"><ha-icon icon="mdi:form-select"></ha-icon><span>${this.t('editor.visual_editor')}<small>${this.t('editor.guided')}</small></span></button><button data-mode="yaml" role="tab"><ha-icon icon="mdi:code-braces"></ha-icon><span>YAML<small>${this.t('editor.advanced')}</small></span></button></div><form class="visual-form">${this.editorSections(type,initial)}</form><div class="yaml-editor" style="display:none"><ha-alert alert-type="warning">${this.t('editor.yaml_warning')}</ha-alert><textarea spellcheck="false" aria-label="${this.t('editor.yaml_label')}">${this.escape(this.toYaml(raw))}</textarea></div><ha-alert class="editor-error" alert-type="error" style="display:none"></ha-alert></div><ha-dialog-footer slot="footer"><ha-button slot="secondaryAction" appearance="plain">${this.t('actions.cancel')}</ha-button><ha-button slot="primaryAction" appearance="accent">${this.t('actions.save')}</ha-button></ha-dialog-footer>`;document.body.appendChild(dialog);
    ...
    syncMode();updateLightMode();updateControlBehavior();['cover_control_mode','cover_position_feedback','cover_movement_feedback','cover_stop_enabled','cover_tilt_enabled','control_mode'].forEach(k=>{if(form.elements[k])form.querySelectorAll(`[name="${k}"]`).forEach(input=>input.onchange=syncMode);});

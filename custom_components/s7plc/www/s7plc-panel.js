...
const COVER_UI_FROM_ENTITY = entity => {
  const control=entity.position_state_address?"position":"traditional";
  const positionFeedback=control==="traditional"&&entity.use_state_topics===true?"endstops":"timed";
  const movementFeedback=entity.cover_status_address?"status":control==="traditional"&&(entity.cover_opening_address||entity.cover_closing_address||entity.cover_stopped_address)?"bits":"none";
...
const COVER_VIRTUAL_FIELDS=["cover_control_mode","cover_position_feedback","cover_movement_feedback","cover_stop_enabled","cover_tilt_enabled","cover_mode","feedback_mode"];
...
const COVER_POSITION_FIELDS=["position_state_address","position_command_address","invert_position","command_bidirectional","tilt_state_address","tilt_command_address","invert_tilt","stop_command_address","stop_pulse_duration"];
const CLEAN_COVER_ENTITY=(source,ui)=>{const entity={...source};COVER_VIRTUAL_FIELDS.forEach(key=>delete entity[key]);if(ui.cover_control_mode==="traditional"){COVER_POSITION_FIELDS.forEach(key=>delete entity[key]);if(ui.cover_position_feedback==="endstops"){entity.use_state_topics=true;}else{entity.use_state_topics=false;delete entity.opening_state_address;delete entity.closing_state_address;} }else{COVER_TRADITIONAL_FIELDS.forEach(key=>delete entity[key]);}
  if(ui.cover_movement_feedback==="none"){["cover_opening_address","cover_closing_address","cover_stopped_address","cover_status_address",...COVER_STATUS_VALUE_FIELDS].forEach(key=>delete entity[key]);}else if(ui.cover_movement_feedback==="bits"){["cover_status_address",...COVER_STATUS_VALUE_FIELDS].forEach(key=>delete entity[key]);}else {["cover_opening_address","cover_closing_address","cover_stopped_address"].forEach(key=>delete entity[key]);}
...
const COMMON = [
  ["name"], ["area"], ["scan_interval","number"]
];
const FIELDS = {
...
const MODE_HIDDEN = {
  covers: {
    position:    ["open_command_address","close_command_address","opening_state_address","closing_state_address","cover_opening_address","cover_closing_address","cover_stopped_address","operate_time","use_state_topics","toggle_mode"],
    traditional: ["position_state_address","position_command_address","stop_command_address","stop_pulse_duration","tilt_state_address","tilt_command_address","invert_tilt","invert_position","command_bidirectional"]
  },
  climates: {
...
// Same idea as CLIMATE_PRESET_CORE_DEFAULTS, mirrored on the status-matching
// side: hvac_status_off/heating/cooling_values also carry non-empty
// historical defaults ("0"/"1"/"2"), unlike idle/drying/fan/preheating/
// defrosting which default to "".
...
class S7PlcConfigurationPanel extends HTMLElement {
  ...
  entityCards(entry){const type=this.type||TYPES[0],items=entry.entities[type];if(!items.length)return `<div class="empty small"><ha-icon icon="mdi:playlist-plus"></ha-icon><h3>${this.t('common.empty')}</h3><p>${this.t('common.empty_help')}</p></div>`;return items.map((item,i)=>{const entityId=entry.entity_ids?.[type]?.[i],selected=this.selectedIndices.has(i);return `<article class="${selected?'selected':''}"><label class="entity-select" title="${this.bt('select_entity')}"><input type="checkbox" data-select="${i}" aria-label="${this.bt('select_entity')}" ${selected?'checked':''}><span></span></label><div class="entity-icon"><ha-icon icon="mdi:${this.icon(type)}"></ha-icon></div><div class="details"><b>${this.escape(item.name||item.address||item.state_address||item.current_temperature_address||`${this.t('common.entity')} ${i+1}`)}</b><code>${this.escape(item.address||item.state_address||item.current_temperature_address||item.source_entity||'—')}</code><div>${this.chips(item,type)}</div></div>${entityId?`<span class="state-badge" data-entity-id="${this.escape(entityId)}" title="${this.escape(entityId)}">${this.escape(this.stateText(entityId))}</span>`:''}<button data-edit="${i}" class="icon-btn" title="${this.t('actions.edit')}"><ha-icon icon="mdi:pencil"></ha-icon></button><button data-delete="${i}" class="icon-btn danger" title="${this.t('actions.delete')}"><ha-icon icon="mdi:delete"></ha-icon></button></article>`;}).join('');}
  ...
  inferred(item,type){const copy={...item};if(type==='covers')Object.assign(copy,COVER_UI_FROM_ENTITY(item));if(type==='climates'){Object.assign(copy,CLIMATE_UI_FROM_ENTITY(item));copy.preset_mode_bidirectional=item.preset_mode_bidirectional===true;}if(type==='switches'||type==='lights')copy.control_behavior=CONTROL_MODE_FROM_ENTITY(item);if(type==='lights'){copy.light_mode=LIGHT_MODE_FROM_ENTITY(item);if(copy.light_mode==='dimmable'&&copy.brightness_scale==null)copy.brightness_scale=255;}return copy;}
  ...
  connectionBadgeContent(status){return `<span class="connection-badge-state">${this.t(`common.${status}`)}</span><ha-icon icon="mdi:information-outline"></ha-icon><span class="connection-badge-details">${this.t('connection_details.title')}</span>`;}
  ...
  editorSections(type,item){const fields=FIELDS[type],byKeys=keys=>keys.map(key=>fields.find(field=>field[0]===key)).filter(Boolean),isAddress=f=>f[0].includes('address')||f[0]==='source_entity'||f[0]==='cover_mode'||f[0]==='control_mode',isIdentity=f=>['name','area','scan_interval'].includes(f[0]),section=(icon,title,description,list,key='')=>list.length?`<section class="form-section" ${key?`data-section="${key}"`:''}><div class="section-head"><span class="section-icon"><ha-icon icon="mdi:${icon}"></ha-icon></span><div><b>${title}</b><small>${description}</small></div></div><div class="field-grid">${list.map(f=>this.field(f,item,type)).join('')}</div></section>`:'';
    if(type==='covers')return section('tune-variant',this.t('entity_types.covers.modes.control.title'),this.t('entity_types.covers.modes.control.description'),byKeys(['cover_control_mode']),'cover-control')+section('map-marker-distance',this.t('entity_types.covers.modes.position_feedback.title'),this.t('entity_types.covers.modes.position_feedback.description'),byKeys(['cover_position_feedback']),'cover-position-feedback')+section('motion-sensor',this.t('entity_types.covers.modes.movement_feedback.title'),this.t('entity_types.covers.modes.movement_feedback.description'),byKeys(['cover_movement_feedback']),'cover-movement-feedback')+section('connection',this.t('sections.plc_connection.title'),this.t('sections.plc_connection.description'),byKeys(['open_command_address','close_command_address','toggle_mode','opening_state_address','closing_state_address','operate_time','position_state_address','position_command_address','invert_position','command_bidirectional','cover_opening_address','cover_closing_address','cover_stopped_address','cover_status_address','cover_status_open_values','cover_status_closed_values','cover_status_opening_values','cover_status_closing_values','cover_status_stopped_values']),'addresses')+section('stop-circle-outline',this.t('entity_types.covers.modes.stop.title'),this.t('entity_types.covers.modes.stop.description'),byKeys(['cover_stop_enabled','stop_command_address','stop_pulse_duration']),'cover-stop')+section('rotate-3d-variant',this.t('entity_types.covers.modes.tilt.title'),this.t('entity_types.covers.modes.tilt.description'),byKeys(['cover_tilt_enabled','tilt_state_address','tilt_command_address','invert_tilt']),'cover-tilt')+section('card-account-details-outline',this.t('sections.ha_details.title'),this.t('sections.ha_details.description'),byKeys(['name','area','device_class','scan_interval']),'ha');
    if(type==='climates')return section('tune-variant',this.t('entity_types.climates.sections.control.title'),this.t('entity_types.climates.sections.control.description'),byKeys(['control_mode']),'climate-control')+section('thermometer',this.t('entity_types.climates.sections.temperature.title'),this.t('entity_types.climates.sections.temperature.description'),byKeys(['current_temperature_address','target_temperature_address','min_temp','max_temp','temp_step']),'climate-temperature')+section('radiator',this.t('entity_types.climates.sections.direct_function.title'),this.t('entity_types.climates.sections.direct_function.description'),byKeys(['climate_direct_function','heating_output_address','cooling_output_address']),'climate-direct-function')+section('motion-sensor',this.t('entity_types.climates.sections.direct_feedback.title'),this.t('entity_types.climates.sections.direct_feedback.description'),byKeys(['climate_direct_feedback','heating_action_address','cooling_action_address']),'climate-direct-feedback')+section('tune',this.t('entity_types.climates.sections.mode_control.title'),this.t('entity_types.climates.sections.mode_control.description'),byKeys(['climate_mode_control','on_off_address','preset_mode_address']),'climate-mode-control')+section('format-list-checks',this.t('entity_types.climates.sections.available_modes.title'),this.t('entity_types.climates.sections.available_modes.description'),byKeys(CLIMATE_PRESET_VALUE_FIELDS),'climate-modes')+section('sync',this.t('entity_types.climates.sections.mode_feedback.title'),this.t('entity_types.climates.sections.mode_feedback.description'),byKeys(['preset_mode_bidirectional']),'climate-mode-feedback')+section('list-status',this.t('entity_types.climates.sections.action_feedback.title'),this.t('entity_types.climates.sections.action_feedback.description'),byKeys(['climate_action_feedback','hvac_status_address',...CLIMATE_STATUS_VALUE_FIELDS]),'climate-action-feedback')+section('card-account-details-outline',this.t('sections.ha_details.title'),this.t('sections.ha_details.description'),byKeys(['name','area','scan_interval']),'ha');
    if(type==='switches')return section('tune-variant',this.t('sections.command_behavior.title'),this.t('sections.command_behavior.description'),byKeys(['control_behavior']),'command')+section('connection',this.t('sections.plc_connection.title'),this.t('sections.plc_connection.description'),byKeys(['state_address','command_address']),'addresses')+section('cog-outline',this.t('sections.options.title'),this.t('sections.options.description'),byKeys(['pulse_duration']),'options')+section('card-account-details-outline',this.t('sections.ha_details.title'),this.t('sections.ha_details.description'),byKeys(['name','area','scan_interval']),'ha');
    if(type==='lights')return section('tune-variant',this.t('sections.command_behavior.title'),this.t('sections.command_behavior.description'),byKeys(['control_behavior']),'command')+section('lightbulb-outline',this.t('entity_types.lights.mode.title'),this.t('entity_types.lights.mode.description'),byKeys(['light_mode']),'light-mode')+section('connection',this.t('sections.plc_connection.title'),this.t('sections.plc_connection.description'),byKeys(['state_address','command_address','brightness_state_address','brightness_command_address']),'addresses')+section('cog-outline',this.t('sections.options.title'),this.t('sections.options.description'),byKeys(['pulse_duration','brightness_scale']),'options')+section('card-account-details-outline',this.t('sections.ha_details.title'),this.t('sections.ha_details.description'),byKeys(['name','area','scan_interval']),'ha');
  ...
  openEditor(index=null){const entry=this.entries.find(e=>e.entry_id===this.entryId),type=this.type||TYPES[0],raw=index===null?{}:entry.entities[type][index],initial=this.inferred(raw,type),dialog=document.createElement('ha-dialog');dialog.open=true;dialog.headerTitle=index===null?this.t('editor.new_entity'):this.t('editor.edit_entity');dialog.style.setProperty('--mdc-dialog-max-width','min(940px,95vw)');dialog.style.setProperty('--mdc-dialog-min-width','min(940px,95vw)');dialog.style.setProperty('--dialog-content-padding','0');dialog.innerHTML=`<style>${this.dialogStyles}</style><div class="dialog-body"><div class="editor-intro"><span class="editor-type-icon"><ha-icon icon="mdi:${this.icon(type)}"></ha-icon></span><div><span class="eyebrow">${this.t(`entity_types.${type}.label`)}</span><h3>${index===null?this.t('editor.configure_new'):this.t('editor.update_configuration')}</h3><p>${this.t('editor.save_help')}</p></div></div><div class="mode-tabs" role="tablist" aria-label="${this.t('editor.editor_mode')}"><button class="active" data-mode="visual" role="tab"><ha-icon icon="mdi:form-select"></ha-icon><span>${this.t('editor.visual_editor')}<small>${this.t('editor.guided')}</small></span></button><button data-mode="yaml" role="tab"><ha-icon icon="mdi:code-braces"></ha-icon><span>YAML<small>${this.t('editor.advanced')}</small></span></button></div><form class="visual-form">${this.editorSections(type,initial)}</form><div class="yaml-editor" style="display:none"><ha-alert alert-type="warning">${this.t('editor.yaml_warning')}</ha-alert><textarea spellcheck="false" aria-label="${this.t('editor.yaml_label')}">${this.escape(this.toYaml(raw))}</textarea></div><ha-alert class="editor-error" alert-type="error" style="display:none"></ha-alert></div><ha-dialog-footer slot="footer"><ha-button slot="secondaryAction" appearance="plain">${this.t('actions.cancel')}</ha-button><ha-button slot="primaryAction" appearance="accent">${this.t('actions.save')}</ha-button></ha-dialog-footer>`;document.body.appendChild(dialog);
    ...
    const updateLightMode=()=>{if(type!=='lights')return;const dimmable=form.querySelector('input[name="light_mode"]:checked')?.value==='dimmable';['brightness_state_address','brightness_command_address','brightness_scale'].forEach(key=>form.querySelector(`[data-field="${key}"]`)?.classList.toggle('hidden-field',!dimmable));if(dimmable&&form.elements.brightness_scale&&!form.elements.brightness_scale.value)form.elements.brightness_scale.value='255';updateOptionsSection();};
    syncMode();updateLightMode();updateControlBehavior();['cover_control_mode','cover_position_feedback','cover_movement_feedback','cover_stop_enabled','cover_tilt_enabled','control_mode','climate_direct_function','climate_direct_feedback','climate_mode_control','climate_action_feedback'].forEach(k=>{if(form.elements[k])form.querySelectorAll(`[name="${k}"]`).forEach(input=>input.onchange=()=>{if(type==='climates'){const changed=new Set((form.dataset.climateChanged||'').split(',').filter(Boolean));changed.add(k);form.dataset.climateChanged=[...changed].join(',');}syncMode();});});
    ...
    if(form.elements.toggle_mode)form.elements.toggle_mode.onchange=syncMode;
...
.light-options .control-card,.compact-control-card{min-height:110px;padding:12px 14px!important;gap:6px!important}.cover-options{grid-template-columns:repeat(auto-fit,minmax(135px,1fr))}.compact-control-card{align-items:center!important;text-align:center}.compact-control-card small{margin-top:3px}
.light-options .control-card small{margin-top:3px}
.visual-form label:not(.check){display:flex;flex-direction:column;gap:8px;font-size:13px;overflow-wrap:break-word}
...
.visual-form .check{display:flex;flex-direction:column;justify-content:center;gap:4px;min-height:52px;padding:8px 14px;border-radius:13px;background:var(--card-background-color);border:1px solid var(--divider-color);cursor:pointer;transition:border-color .15s,box-shadow .15s;overflow-wrap:break-word}
.visual-form .check .check-row{display:flex;align-items:center;justify-content:space-between;gap:12px}
.visual-form .check .check-row>b{min-width:0}

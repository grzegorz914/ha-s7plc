...
class S7PlcConfigurationPanel extends HTMLElement {
  ...
  openConfigurationEditor(){
    ...
    dialog.innerHTML=`<style>${this.dialogStyles}</style><div class="dialog-body configuration-editor"><ha-alert alert-type="warning">${this.t('configuration_yaml_warning')}</ha-alert><div class="configuration-tools"><button id="yaml-import"><ha-icon icon="mdi:upload"></ha-icon>${this.t('import_yaml')}</button><button id="yaml-export"><ha-icon icon="mdi:download"></ha-icon>${this.t('export_yaml')}</button><input id="yaml-file" type="file" accept=".yaml,.yml,text/yaml,application/yaml" hidden></div><textarea spellcheck="false" aria-label="${this.t('configuration_yaml')}">${this.escape(entry.configuration_yaml||'')}</textarea><ha-alert class="editor-error" alert-type="error" style="display:none"></ha-alert></div><ha-dialog-footer slot="footer"><ha-button slot="secondaryAction" appearance="plain">${this.t('cancel')}</ha-button><ha-button slot="primaryAction" appearance="accent">${this.t('save')}</ha-button></ha-dialog-footer>`;
    ...
    dialog.querySelector('#yaml-export').onclick=()=>{const blob=new Blob([textarea.value],{type:'application/yaml;charset=utf-8'}),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=`${(entry.title||'s7plc').replace(/[^a-z0-9_-]+/gi,'-').toLowerCase()}-config.yaml`;link.click();URL.revokeObjectURL(url);};
    dialog.querySelector('[slot=secondaryAction]').onclick=()=>{dialog.open=false;};dialog.addEventListener('closed',()=>dialog.remove());
    dialog.querySelector('[slot=primaryAction]').onclick=async()=>{const alert=dialog.querySelector('.editor-error');try{await this._hass.callWS({type:'s7plc/config/save_configuration',entry_id:this.entryId,configuration_yaml:textarea.value});dialog.open=false;this.selectedIndices.clear();this._loaded=false;await this.load();}catch(err){alert.textContent=err.message||String(err);alert.style.display='block';}};
  }
...
.connection-badge{display:inline-flex;align-items:center;gap:7px;padding:4px 12px;border-radius:99px;background:#ffffff1f;box-shadow:inset 0 0 0 1px #ffffff2e;color:#fff;font-size:11px;font-weight:700;letter-spacing:.03em;line-height:1.5}
...
@media(prefers-reduced-motion:reduce){.dialog-body *,.dialog-body *::after{transition:none!important;animation:none!important}}
@media(max-width:650px){.dialog-body{max-height:66vh;padding:0 14px 16px}.editor-intro p{display:none}.form-section{padding:14px 12px}.field-grid{grid-template-columns:1fr}.control-options{grid-template-columns:1fr}.control-card{min-height:0;flex-direction:row!important}.mode-tabs button{font-size:12px}.mode-tabs button small{display:none}}`;}
}

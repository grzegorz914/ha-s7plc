...
const CLEAN_COVER_ENTITY=(source,ui)=>{const entity={...source};COVER_VIRTUAL_FIELDS.forEach(key=>delete entity[key]);if(ui.cover_control_mode==="traditional"){COVER_POSITION_FIELDS.forEach(key=>delete entity[key]);if(ui.cover_position_feedback==="endstops"){entity.use_state_topics=Boolean(source.use_state_topics&&entity.opening_state_address&&entity.closing_state_address);}else{entity.use_state_topics=false;delete entity.opening_state_address;delete entity.closing_state_address;} }else{COVER_TRADITIONAL_FIELDS.forEach(key=>delete entity[key]);}

...
const COVER_UI_FROM_ENTITY = entity => {
  ...
  const positionFeedback=isTraditionalLike?(entity.cover_position_feedback||legacyPositionFeedback):"timed";
  const movementBitsPresent=entity.cover_opening_address||entity.cover_closing_address||entity.cover_stopped_address;
  // toggle_mode treats position and movement feedback as independently
  // selectable - a status word chosen for one purpose must not be assumed
  // to also cover the other, but a shared cover_status_address still
  // defaults to serving movement too (matches the pre-selector legacy
  // behavior) unless position_feedback already claims it AND there's a
  // genuine alternative (movement bits) to fall back to instead. Plain
  // traditional covers keep the original coupled inference (bits are only
  // ever considered when position feedback isn't already "status") since
  // their runtime doesn't implement the independent-source model.
  const movementFeedback = control==="toggle"
    ? (entity.cover_status_address&&!(positionFeedback==="status"&&movementBitsPresent)?"status":movementBitsPresent?"bits":"none")
...
const CLEAN_COVER_ENTITY=(source,ui)=>{const entity={...source};COVER_VIRTUAL_FIELDS.forEach(key=>delete entity[key]);const isTraditionalLike=ui.cover_control_mode==="traditional"||ui.cover_control_mode==="toggle";const isToggle=ui.cover_control_mode==="toggle";if(ui.preserve_legacy&&isTraditionalLike){COVER_POSITION_FIELDS.forEach(key=>delete entity[key]);delete entity.cover_position_feedback;return entity;}if(isTraditionalLike){COVER_POSITION_FIELDS.forEach(key=>delete entity[key]);const mode=ui.cover_position_feedback;entity.cover_position_feedback=mode;if(mode==="status"){delete entity.use_state_topics;delete entity.opening_state_address;delete entity.closing_state_address;
      ...
      // the independent-source model toggle_mode uses. toggle_mode itself
      // leaves this to the shared scoping pass below instead.
      if(!isToggle)["cover_opening_address","cover_closing_address","cover_stopped_address"].forEach(key=>delete entity[key]);}else{if(!isToggle&&ui.cover_movement_feedback!=="status")["cover_status_address",...COVER_STATUS_VALUE_FIELDS].forEach(key=>delete entity[key]);if(mode==="timed"){entity.use_state_topics=false;delete entity.opening_state_address;delete entity.closing_state_address;}else{entity.use_state_topics=true;if(mode==="opening")delete entity.closing_state_address;if(mode==="closing")delete entity.opening_state_address;}}}else{COVER_TRADITIONAL_FIELDS.forEach(key=>delete entity[key]);delete entity.cover_position_feedback;}
  if(isToggle){
    ...
    // Original coupled precedence for traditional (and position, which
    // never had an independent movement selector to begin with): exactly

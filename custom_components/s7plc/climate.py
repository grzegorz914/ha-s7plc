...
async def async_setup_entry(
    ...
    for item in entry.options.get(CONF_CLIMATES, []):
        ...
        if control_mode == CONTROL_MODE_DIRECT:
            ...
            # it back into hvac_mode) follows preset_mode_bidirectional
            # above, so only poll it when actually needed.
            ...
            hvac_status_defrosting_values = item.get(
                CONF_HVAC_STATUS_DEFROSTING_VALUES,
                DEFAULT_HVAC_STATUS_DEFROSTING_VALUES,
            )
...
class S7ClimateSetpointControl(
    ...
    def __init__(
        ...
        # whose value is None (left empty) is removed from the thermostat's
        # selectable HVAC mode list entirely - except OFF when on_off_address
        # is configured, since that address is then always able to turn the
        ...
        self._attr_hvac_modes = [
            ...
            if self._preset_mode_values[mode] is not None
        ]
        ...
        if on_off_address or self._preset_mode_values[HVACMode.OFF] is not None:
        ...
        # with a real (non-disabled) preset value are included, so a
        # disabled mode (empty value) - including OFF when it's actually
        # handled via on_off_address - can't be mistaken for a real one.
    ...
    def hvac_mode(self) -> HVACMode | None:
        ...
        configured. preset_mode_address, by contrast, is write-only by
        default for backward compatibility -- reading a target/command
    ...
    def hvac_action(self) -> HVACAction | None:
        ...
        hvac_status_defrosting_values/hvac_status_idle_values). Otherwise
        infer from target vs current temperature comparison.
        """
    ...
    def extra_state_attributes(self):
        ...
        if self._preset_mode_address:
            ...
            attrs["preset_mode_bidirectional"] = self._preset_mode_bidirectional
        if self._hvac_status_address:
    ...
    async def async_set_hvac_mode(self, hvac_mode: HVACMode) -> None:
        ...
        # preset_mode_fan_only_value). Skipped if that mode's value is None
        # (left empty): this happens for OFF when it's only selectable
        # thanks to on_off_address, in which case there is no real PLC value
        # to write here.
        if self._preset_mode_address:

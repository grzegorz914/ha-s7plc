...
def test_migrate_folds_legacy_scale_fields_into_address_at_startup(monkeypatch):
    ...
    # uid migration was a no-op (uid already set); only the scale migration
    # fired.
    assert len(update_calls) == 1
    new_options = update_calls[0][1]["options"]
    sensor_item = new_options["sensors"][0]
    ...
    assert sensor_item["uid"] == "already-set"
...
def test_migrate_folds_legacy_brightness_scale_into_address_at_startup(monkeypatch):
    ...
    # uid migration was a no-op (uid already set); only the brightness-scale
    # migration fired.
    assert len(update_calls) == 1
    new_options = update_calls[0][1]["options"]
    ...
    assert light_item["uid"] == "already-set"
...
def test_migrate_folds_legacy_value_multiplier_into_address_at_startup(monkeypatch):
    ...
    # uid migration was a no-op (uid already set); only the multiplier
    # migration fired.
    assert len(update_calls) == 1
    new_options = update_calls[0][1]["options"]
    sensor_item = new_options["sensors"][0]
